		...
		<div id="wrap">
	      	...
	      	<div class="container" >
	        	...
	        	<div class="row" style="padding-left:20px;">
	        		<input type="hidden" id="action" name="action" value="getCommentsFromTarget" />
		        	target_id: <input type="text" id="target_id" name="target_id" />
...
$(document).ready(function(){
  	...
  	var getCommentsFromTarget = function() {
	  	...
	  	tr.append($("<td> associate </td>"));

	  	tbody.append(tr);

  		$.getJSON("../php/takeAction.php", {"target_id" : target_id, "action" : action}, function(res) {
	  		for( var i = 0; i < res.length; i ++ ) {
	  			var user = jQuery.parseJSON(res[i]);
	  			var tr = $("<tr></tr>");

	  			tr.append($("<td>" + user["user_id"] 	+ "</td>"));
	  			tr.append($("<td>" + user["content"] 	+ "</td>"));
	  			tr.append($("<td>" + user["associate"] 	+ "</td>"));
	  			tbody.append(tr);
	  		}
  	...
  	submit.onclick = getCommentsFromTarget;
});
